...
function echoColor() {
    ...
    "purple")
        ...
        #
    "yellowBlack")
        # Carácter amarillo de Bottome Negro
...
function checkSystemForUpdate() {
	...
	if [[ -z ${release} ]]; then
		echoColor red "\nEste script no admite este sistema\n"
...
function printMsg(){
	...
	echoColor yellow "--------------------------------------------"
	echo ""
	echo -e  "\033[1;;35m1* [\033[0m\033[31mv2rayN/Matsuri/nekobox/nekoray\033[0m\033[1;;35m] usar hysteria coreCorrer directamente: \033[0m"
	echoColor green "Salida del archivo de configuración del cliente a: `pwd`/Hys-${remarks}(v2rayN).json ( Descargue el archivo de configuración generado directamente [recomendado] / Copie y péguelo usted mismo y configelo en el área local )"
	echoColor green "Tips:El cliente solo se abre de forma predeterminada http(10809)、socks5(10808)! Por favor, consulte otras formas hysteria, Modifique el cliente por el documento usted mismo config.json"
	echoColor skyBlue "↓****↓↓↓copy↓↓↓****↓"
	cat ./Hys-${remarks}\(v2rayN\).json
	echoColor skyBlue "↑****↑↑↑copy↑↑↑****↑\n"
	url=`cat /etc/hihy/result/url.txt`
	echo -e  "\033[1;;35m2* [\033[0m\033[31mShadowrocket/Matsuri/nekobox/Passwall/HTTP Injector\033[0m\033[1;;35m] 一 URL: \033[0m"
	echoColor green ${url}
	echo -e "\n"
	echo -e  "\033[1;;35m3* [\033[0m\033[31mClash.Meta\033[0m\033[1;;35m] El archivo de configuración `pwd`/Hys-${remarks}(clashMeta).yaml , descargarlo al cliente para usar(beta)\033[0m"
	echoColor yellow "--------------------------------------------"
}
...
function changeIp64(){
    if [ ! -f "/etc/hihy/conf/hihyServer.json" ]; then
  		echoColor red "No se ha instalado correctamente hihy!"
        exit
	fi 
	now=`cat /etc/hihy/conf/hihyServer.json | grep "resolve_preference"`
    case ${now} in 
		*"64"*)
			echoColor purple "IPv6 actual preferido"
            echoColor yellow " \n->Configurar la prioridad IPv4 más alta que IPv6? (S/N, N)"
            read input
            if [ -z "${input}" ];then
		...
		;;
		*"46"*)
			echoColor purple "Preferido IPv4 actual"
            echoColor yellow " \n->Establezca la prioridad IPv6 más alta que IPv4? (S/N, N)"
            read input
            if [ -z "${input}" ];then
                echoColor green "Ignore."
                exit
...
function getPortBindMsg(){
        ...
        if [ "${msg}" == "" ];then
				...
				command=`echo ${msg} | awk '{print $1}'`
          		...
          		echoColor purple "Port: ${1}/${2} Ha sido ${command}(${name}) Occupy, Process Pid es: ${pid}."
  				echoColor green "¿Cierra automáticamente la ocupación del puerto? (S/N)"
				read bindP
				if [ -z "${bindP}" ];then
					echoColor red "Debido a la ocupación del puerto, la instalación de salida.Cierre o reemplace el puerto manualmente..."
					if [ "${1}" == "TCP" ] && [ "${2}" == "80" ];then
						echoColor "Si no puede cerrar el puerto de $ {1}/$ {2}, utilice otros certificados para obtener"
					fi
					exit
				elif [ "${bindP}" == "y" ] ||  [ "${bindP}" == "Y" ];then
					kill -9 ${pid}
					echoColor purple "Desvinculante..."
					sleep 3
					if [ $1 == "TCP" ]; then
						msg=`lsof -i ${1}:${2} | grep LISTEN`
					else
        			...
        			if [ "${msg}" != "" ];then
						echoColor red "El cierre de ocupación del puerto falló. Después de cerrar el proceso a la fuerza, se reinicia. Compruebe si hay un proceso ..."
						exit
					else
						echoColor green "Desvinculación del puerto exitosa..."
					fi
				else
...
function setHysteriaConfig(){
	...
	echo -e "\033[32mSeleccione el método de solicitud de certificado:\n\n\033[0m\033[33m\033[01m1、Utilice ACME para realizar la solicitud (recomendado, es necesario abrir tcp 80/443)\n2. Utilice el archivo de certificado local\n3. Certificado autofirmado\033[0m\033[32m\n\nIngrese el número de serie:\033[0m"
    read certNum
	useAcme=false
	useLocalCert=false
	if [ -z "${certNum}" ] || [ "${certNum}" == "3" ];then
		echo -e  "\nNota: Los certificados autofirmados han estado sujetos a una gran cantidad de bloques aleatorios en el período reciente sin obfs."
		echoColor red "Si debe utilizar un certificado autofirmado, configure la verificación de ofuscación de obfs en la configuración siguiente para garantizar la seguridad."
		echoColor green "Ingrese el nombre de dominio del certificado autofirmado (predeterminado: wechat.com):"
		read domain
		if [ -z "${domain}" ];then
			domain="wechat.com"
		fi
		...
		echoColor green "Determinar si la dirección utilizada por el cliente para conectarse es correcta IP de red pública:"`echoColor red ${ip}`"\n"
		while true
		do	
			echo -e "\033[32mSeleccione:\n\n\033[0m\033[33m\033[01m1, correcto (predeterminado)\n2, incorrecto, ingrese manualmente ip\033[0m\033[32m\n\nIngrese el número de serie:\033[0m"
			read ipNum
			if [ -z "${ipNum}" ] || [ "${ipNum}" == "1" ];then
				break
			elif [ "${ipNum}" == "2" ];then
				echoColor green "Ingrese la IP pública correcta (no es necesario agregar [] a la dirección IPv6)):"
				read ip
				if [ -z "${ip}" ];then
					echoColor red "Errores de entrada, por favor vuelva a ingresar..."
					continue
				fi
				break
			else
				echoColor red "\n->Errores de entrada, por favor vuelva a ingresar:"
			fi
		done		
		cert="/etc/hihy/cert/${domain}.crt"
		key="/etc/hihy/cert/${domain}.key"
		useAcme=false
		echoColor purple "\n\n->Ha elegido el cifrado del certificado ${domain} autofirmado. IP de la red pública: "`echoColor red ${ip}`"\n"
		echo -e "\n"
    elif [ "${certNum}" == "2" ];then
		echoColor green "Ingrese la ruta del archivo del certificado (se requiere un certificado de cadena completa y se proporciona la cadena de certificados completa):"
		read cert
		while :
		do
			if [ ! -f "${cert}" ];then
				echoColor red "\n\n->La ruta no existe, ¡vuelve a ingresar!"
				echoColor green "Por favor ingrese la ruta del archivo del certificado:"
				read  cert
			else
				break
			fi
		done
		echo -e "\n\n->cert ruta de archivo: "`echoColor red ${cert}`"\n"
		echoColor green "Ingrese la ruta del archivo de clave del certificado:"
		read key
		while :
		do
			if [ ! -f "${key}" ];then
		...
		echo -e "\n\n->ruta del archivo clave: "`echoColor red ${key}`"\n"
    	...
    	echoColor green "Ingrese el nombre de dominio (debe resolverse correctamente en la máquina local, desactive CDN):"
		read domain
		while :
		do
			if [ -z "${domain}" ];then
		...
		while :
		do	
			echoColor purple "\n->Detectar ${domain}, resolución DNS..."
			ip_resolv=`dig +short ${domain} A`
			if [ -z "${ip_resolv}" ];then
				ip_resolv=`dig +short ${domain} AAAA`
			fi
			if [ -z "${ip_resolv}" ];then
				echoColor red "\n\n->La resolución del nombre de dominio falló y no se obtuvo ningún registro DNS (A/AAAA). Verifique si el nombre de dominio se resolvió correctamente en la máquina local.!"
				echoColor green "Ingrese el nombre de dominio (debe resolverse correctamente en la máquina local, desactive CDN):"
				read  domain
				continue
			fi
			...
			if [ "${localip}" != "${remoteip}" ];then
				echo -e " \n\n->ip: "`echoColor red ${localip}`" \n\n->nombre de dominio / ip: "`echoColor red ${remoteip}`"\n"
				...
				if [ "${localip}" != "${remoteip}" ];then
					echoColor red "\n\n->La IP resuelta por el nombre de dominio no coincide con la IP local; vuelva a ingresarla.!"
					echoColor green "Ingrese el nombre de dominio (debe resolverse correctamente en la máquina local, desactive CDN):"
					read  domain
					continue
				else
		...
		useAcme=true
		echoColor purple "\n\n->El análisis es correcto, utilice el ACME integrado de hysteria para solicitar el certificado.:"`echoColor red ${domain}`"\n"
    fi
	...
	while :
	do
		echoColor green "Ingrese el puerto que desea abrir. Este puerto es el puerto del servidor. Se recomienda que sea 10000-65535. (El valor predeterminado es aleatorio)"
		read  port
		if [ -z "${port}" ];then
			port=$(($(od -An -N2 -i /dev/random) % (65534 - 10001) + 10001))
			echo -e "\n->Usar puerto aleatorio:"`echoColor red ${ut}/${port}`"\n"
		else
			echo -e "\n->El puerto al que ingresaste:"`echoColor red ${ut}/${port}`"\n"
		fi
	...
	if [ "${protocol}" == "udp" ];then
		echoColor green "\n->Se detecta que ha seleccionado el protocolo udp. Puede utilizar la función [Salto de puerto], que es la recomendada."
		echo -e "Tip: Los operadores bloquean/QoS/interrumpen fácilmente las conexiones UDP de un solo puerto a largo plazo. Habilitar esta función puede evitar este problema de manera efectiva.."
		echo -e "\033[32mSeleccione si desea habilitar:\n\n\033[0m\033[33m\033[01m1, habilitar (predeterminado)\n2, omitir\033[0m\033[32m\n\nIngrese el número de serie:\033[0m"
		read portHoppingStatus
		if [ -z "${portHoppingStatus}" ] || [ $portHoppingStatus == "1" ];then
			portHoppingStatus="true"
			echoColor purple "\n->Usted elige habilitar el salto de puerto/portación múltiple(Port Hopping)"
			echo -e "Salto de puerto/multipuerto(Port Hopping)La función requiere varios puertos. Asegúrese de que estos puertos no estén escuchando otros servicios.\nTip: El número de selecciones de puertos no debe ser demasiado. Se recomienda seleccionar alrededor de 1000, entre 1 y 65535. Se recomienda seleccionar un rango de puertos continuo..\n"
			while :
			do
				...
				echo -e "\n->Puerto de salida:"`echoColor red ${portHoppingStart}`"\n"
			...
			echo -e "\n->Salto de puerto/multipuerto de su elección(Port Hopping)Los parámetros son: "`echoColor red ${portHoppingStart}:${portHoppingEnd}`"\n"
		else
    ...
    echoColor green "Ingrese su retraso promedio en este servidor, que está relacionado con la velocidad de reenvío (predeterminado 200, unidad: ms):"
    read  delay
    if [ -z "${delay}" ];then
		delay=200
    fi
	echo -e "\n->Delay: `echoColor red ${delay}`ms\n"
    echo -e "\nVelocidad esperada, que es la velocidad máxima del cliente. El servidor no está limitado por defecto."`echoColor red Tips:El script automáticamente *1.10 hará redundancia. Si sus expectativas son demasiado bajas o demasiado altas, afectará la eficiencia del reenvío. Complete la información con sinceridad.!`
    echoColor green "Ingrese la velocidad de enlace descendente esperada por el cliente: (predeterminado 50, unidad: mbps):"
    read  download
    if [ -z "${download}" ];then
        download=50
    fi
	echo -e "\n->Velocidad de descarga del cliente:"`echoColor red ${download}`"mbps\n"
    echo -e "\033[32mIngrese la velocidad de enlace ascendente esperada del cliente (predeterminado 10, unidad: mbps):\033[0m" 
    read  upload
    if [ -z "${upload}" ];then
        upload=10
    fi
	echo -e "\n->Velocidad de enlace ascendente del cliente:"`echoColor red ${upload}`"mbps\n"
	echoColor green "Ingrese la contraseña de autenticación (generada aleatoriamente de manera predeterminada, se recomienda una contraseña segura de más de 20 caracteres):"
	read auth_secret
	...
	auth_str=""
	obfs_str=""
	echo -e "Tips: Si se utiliza el cifrado de ofuscación obfs, la capacidad antibloqueo es más fuerte y puede identificarse como tráfico UDP desconocido, pero aumentará la carga de la CPU y hará que la velocidad máxima disminuya. Si está buscando rendimiento y no está bloqueado, es No se recomienda usarlo."
	echo -e "\033[32mSeleccione el método de verificación:\n\n\033[0m\033[33m\033[01m1, auth_str (predeterminado)\n2, obfs\033[0m\033[32m\n\nIngrese el número de serie:\033[0m"
	read auth_num
...
EOF
	...
	echoColor green "Ingrese los comentarios del nombre del cliente (distinción de nombre de dominio/IP predeterminado, por ejemplo, ingrese prueba, el nombre es Hys-test):"
	read remarks
...
EOF
	...
	case ${msg} in 
		*"Failed to get a certificate with ACME"*)
			echoColor red "Nombre de dominio: ${u_host}, ¡falló la solicitud del certificado! Verifique si el panel de firewall proporcionado por el servidor está activado (TCP:80,443)\nSi el nombre de dominio está resuelto correctamente en esta IP (¡no abra CDN!)\nSi Los dos puntos anteriores no se pueden cumplir; reinstálelo utilizando un certificado autofirmado."
		...
		*) 	
			...
			echoColor red "Error desconocido: verifique el mensaje de error a continuación y envíe un problema a nuestro canal de Telegram"
...
function changeServerConfig(){
	if [ ! -f "/etc/systemd/system/hihy.service" ]; then
		echoColor red "Instale histeria primero y luego modifique la configuración...."
		exit
	fi
...
function hihyNotify(){
	...
	if [ -z $remoteV ];then
		...
		if [ "${localV}" != "${remoteV}" ];then
			echoColor purple "[Update] hihy tiene una actualización, versión:v${remoteV}, se recomienda actualizar y verificar el registro: https://t.me/vpnmx"
		fi
	fi
...
function hyCoreNotify(){
	if [ -f "/etc/hihy/bin/appS" ]; then
		...
		if [ -z $remoteV ];then
			...
			if [ "${localV}" != "${remoteV}" ];then
				echoColor purple "[Update] histeria ha sido actualizada, versión:${remoteV}. Registro: https://github.com/apernet/hysteria/blob/master/CHANGELOG.md"
			fi
		fi
...
function checkStatus(){
	status=`systemctl is-active hihy`
    if [ "${status}" = "active" ];then
		echoColor green "Hysteria corre normalmente"
	else
...
{	
	if [ -f "/etc/systemd/system/hihy.service" ]; then
		echoColor green "Has instalado histeria correctamente. Si necesitas modificar la configuración, utiliza la opción 9/12."
		exit
	fi
...
function addPortHoppingNat() {
	...
	if systemctl status firewalld 2>/dev/null | grep -q "active (running)"; then
		...
		if ! [ -x "$(command -v netfilter-persistent)" ]; then
			echoColor red "[Warnning]:netfilter-persistent La instalación falla, pero el progreso de la instalación no se detiene, PortHopping Las reglas de reenvío son reglas temporales y pueden dejar de ser válidas después de reiniciar. ¿Quieres seguir usando las reglas temporales??(y/N)"
		...
		if systemctl status netfilter-persistent 2>/dev/null | grep -q "active (exited)"; then
			...
			echoColor red "netfilter-persistent no se ha iniciado. Las reglas de reenvío PortHopping no se pueden conservar. El reinicio del sistema fallará. Ejecute manualmente el guardado de netfilter-persistent. Continuar ejecutando el script no afectará las configuraciones posteriores..."
		fi
...
function changeMode(){
	if [ ! -f "/etc/hihy/conf/hihyServer.json" ]; then
		echoColor red "El archivo de configuración no existe, salir..."
		exit
	fi
	...
	port=`cat /etc/hihy/conf/hihyServer.json | grep "listen" | awk '{print $2}' | tr -cd "[0-9]"`
	if [ "${protocol}" = "udp" ];then
		echo -e "\033[32m\nSeleccione el tipo de protocolo que desea modificar:\n\n\033[0m\033[33m\033[01m1, faketcp\n2, wechat-video\033[0m\033[32m\n\nIngrese el número de serie:\033[0m"
    	read pNum
		if [ -z "${pNum}" ] || [ "${pNum}" == "1" ];then
			echoColor purple "Elija modificar el tipo de protocolo a faketcp."
		...
		echo -e "\033[32m\nSeleccione el tipo de protocolo a modificar:\n\n\033[0m\033[33m\033[01m1, udp\n2, faketcp\033[0m\033[32m\n\nIngrese el número de serie:\033[0m"
    	read pNum
		if [ -z "${pNum}" ] || [[ "${pNum}" == "1" ]];then
			echoColor purple "Elija modificar el tipo de protocolo a udp."
...
function checkLogs(){
	echoColor purple "Registro de Hysteria en tiempo real, nivel: información, presione Ctrl+C para salir:"
...
function restart(){
	...
	status=`systemctl is-active hihy`
	if [ "${status}" = "active" ];then
		echoColor green "Reiniciar exitosamente"
	else
...
`echoColor yellow "6)  Status"`
`echoColor skyBlue "....................."`
`echoColor yellow "7)  Actualizar Core"`
`echoColor yellow "8)  Ver la configuración actual"`
`echoColor skyBlue "9)  Reconfigurar"`
`echoColor yellow "10) Cambiar prioridad ipv4/ipv6"`
`echoColor yellow "11) Renovar hihy"`
`echoColor red "12) Restablecer Config"`
...
if [ "$1" == "install" ] || [ "$1" == "1" ]; then
	...
	echoColor purple "-> 4) Pausar hysteria"
	stop
elif [ "$1" == "restart" ]  || [ "$1" == "5" ]; then
	echoColor purple "-> 5) Reanudar hysteria"
	restart
elif [ "$1" == "checkStatus" ]  || [ "$1" == "6" ]; then
	echoColor purple "-> 6) Status"
	checkStatus
elif [ "$1" == "updateHysteriaCore" ]  || [ "$1" == "7" ]; then
	echoColor purple "-> 7) Renovar Core"
	updateHysteriaCore
elif [ "$1" == "printMsg" ]  || [ "$1" == "8" ]; then
	echoColor purple "-> 8) Ver la configuración actual"
	printMsg
	...
	echoColor purple "-> 12) Restablecer"
	reinstall
